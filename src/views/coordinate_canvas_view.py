...
class CoordinateCanvasView:
    ...
    def _on_canvas_configure(self, event):
        ...
        if event.widget == self.canvas:
            ...
            if new_width != self.canvas_width or new_height != self.canvas_height:
                print(
                    f"[DEBUG] キャンバスサイズ変更: {self.canvas_width}x{self.canvas_height} → {new_width}x{new_height}"
                )
    ...
    def display_image(
        self, tk_image: ImageTk.PhotoImage, width: int = None, height: int = None
    ):
        """画像を表示"""
        self.clear_canvas()
        ...
        self.current_image = self.canvas.create_image(
            center_x, center_y, anchor="center", image=tk_image
        )
        ...
        print(
            f"[DEBUG] 画像を表示: キャンバスサイズ {canvas_width}x{canvas_height}, 画像を中央({center_x}, {center_y})に配置"
        )

    def add_coordinate_marker(
        self, x: int, y: int, number: int, color: str = "red"
    ) -> int:
        """座標マーカーを追加"""
        # 円マーカー
    ...
    def highlight_coordinate(self, index: int):
        ...
        if 0 <= index < len(self.coordinate_markers):
            ...
            self.highlight_marker = self.canvas.create_oval(
                x - 10, y - 10, x + 10, y + 10, fill="", outline="yellow", width=3
            )
